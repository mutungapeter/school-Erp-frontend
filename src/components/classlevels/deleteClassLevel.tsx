...
import { useDeleteCLassLevelMutation } from "@/redux/queries/classes/classesApi";
interface Props {
    classLevelId: number;
  refetchClassLevels: () => void;
}
const DeleteClassLevel = ({ classLevelId, refetchClassLevels }: Props) => {
  const [isOpen, setIsOpen] = useState(false);
  const [deleteClassLevel, { isLoading: deleting }] = useDeleteCLassLevelMutation();
  const handleOpenModal = () => setIsOpen(true);
  const handleCloseModal = () => setIsOpen(false);
  const handleDelete = async () => {
    const id = classLevelId;
    try {
      await deleteClassLevel(id).unwrap();
             ...
             {deleting && <Spinner />}
              ...
              <div className="flex items-center space-x-6">
                <div className="p-4 rounded-full bg-red-100">
                  <IoWarningOutline className="text-red-500" size={25} />
                </div>
                <p className="lg:text-lg md:text-lg text-sm">
                  Are you sure you want to permanently delete this Class Level ?
...
export default DeleteClassLevel;
